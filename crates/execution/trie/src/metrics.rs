...
impl StorageMetrics {
    ...
    pub fn record_operation<R>(&self, operation: StorageOperation, f: impl FnOnce() -> R) -> R {
        ...
        let result = f();
        OperationMetrics::duration_seconds(operation.as_str()).record(start.elapsed());
        result
    }
    ...
    pub fn record_duration_per_item(
        ...
        {
            let per_item = duration / count_u32;
            let handle = OperationMetrics::duration_seconds(operation.as_str());
            for _ in 0..count {
                handle.record(per_item);
            }
        }
    }
